...
export const Filters = ({ onSearch }) => {
  ...
  useEffect(() => {
    const {
      ...
      hasParking: hadParkingInitial,
      minPrice: minPriceInitial,
      maxPrice: maxPriceInitial,
    } = queryString.parse(window.location.search);

    setPetFriendly(petFriendlyInitial === "true");
    setHasParking(hadParkingInitial === "true");
    setMinPrice(minPriceInitial || "");
    setMaxPrice(maxPriceInitial || "");
  }, []);

  return (
    <div className="max-w-5xl mx-auto my-5 gap-5 flex border-solid border-slate-400 border-2 p-5 rounded-md">
